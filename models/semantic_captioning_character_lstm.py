...
class SemanticCaptioningCharacterLSTM:
    """
    ...
    def __init__(self,recursion_depth:int,path_to_model_weights:Optional[str]=None) -> None:
        ...
        self.semantic_encoder = load('en_core_web_sm')
        a = ord('a')
        z = ord('z')
    ...
    def _predict_next_character(self, meaning:str,contextual_characters:str,temperature:Optional[float],top_p:Optional[float]) -> str:
        """
        ...
        """
        X_meaning = array([self._get_semantic_vector(meaning)])
        X_characters = array([self._pad(self._convert_characters_to_index(contextual_characters))])
        output_vector = self.model.predict((X_meaning,X_characters),verbose=False)
        predicted_index = self._nucleus_decode(output_vector,top_p,temperature) if top_p else self._temperature_decode(output_vector,temperature) if temperature else self._greedy_decode(output_vector) 
